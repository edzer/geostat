...
```{r}
pts$pop = c(299.708, 45.952)
pts$area = c(302.9, 19.2)
pts
```
we see that the class has changed from `SpatialPoints` to `SpatialPointsDataFrame`, the reason being that in addition to geometry (locations), the object now has a `data.frame` slot with attributes. We
can deal with these attributes 
```{r}
pts["pop"]
pts[,"pop"]
pts[["pop"]]
